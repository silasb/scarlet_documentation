...
Scarlet is an application used by the \href{http://biology.osu.edu/}{Center of Life Sciences Education} to assist with various tasks, such as processing exams, administrating workshops, documenting TA progress, and administrating ASC Surveys.
...
Note:
\textit{.ova} format does not require you to use VirtualBox and isn’t exclusive to VirtualBox.  The \textit{.ova} format is an industrial standard that should be supported by any enterprise virtualization software available after September 2007.  If your virtualization software does not support \textit{.ova} format then you need to contact your virtualization software vendor for assistance.  This documentation is a written guide to installing Scarlet on ViritualBox.

\subsection{Requirements}

\begin{itemize}
\item A computer that can run VirtualBox 4.0.4 or greater.
\item Able to assign a public IP Address to the appliance.
\item Require port 80 to be opened to and from the internet.
Optionally
\item Setup a domain name to point to this public interface.
\end{itemize}

Note:
The server acquires its IP address via a DHCP server.  You can assign a static IP address via the MAC address for a DHCP server.  The MAC address for a VirtualBox appliance can be found in the Networking Settings for the appliance.

\subsection{Time Required}

With a recent machine created in the last two years, it should take at most two hours to install.
...
Installing Scarlet requires that you are able to retrieve two IP addresses from your DHCP provider.  If do know what these terms mean then you need to contact your network administrator for help.
...
To fix this follow section~\ref{sec:configuration} on page~\pageref{sec:configuration}.
